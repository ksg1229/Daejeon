...
DB_CONFIG = {
    'user': 'daejeon',
    'password': 'YOUR_DB_PASSWORD', # os.getenv('DB_PASSWORD') 권장
    'host': '192.168.0.2',
    'port': 1521,
    'sid': 'xe'
}
...
CURRENT_YEAR = 2025
KAKAO_API_KEY = "YOUR_KAKAO_API_KEY" # os.getenv('KAKAO_API_KEY') 권장
try:
...
YEAR_OPTIONS_SINGLE = sorted([y for y in df_data['YEAR'].unique().astype(str).tolist() if y != '2020'])
...
def plot_neighborhood_comparison(target_match_key, energy_metric_name):
    ...
    def calculate_distance(row, target_coords):
        ...
        return geodesic(target_coords, (row['LATITUDE'], row['LONGITUDE'])).meters
...
def handle_address_search(n_clicks, search_query):
    ...
    def calculate_distance(row):
        ...
        return geodesic(target_coords, building_coords).meters
...
if __name__ == '__main__':
    ...
    # app.run(host='0.0.0.0', debug=True, port=8050, dev_tools_ui=False)
    app.run(host='0.0.0.0', port=8050, dev_tools_ui=False)
